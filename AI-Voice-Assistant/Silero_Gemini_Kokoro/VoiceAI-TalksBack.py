...
# Gemini setup
GEMINI_API_KEY = "ENTER_API_KEY"  # Replace with your actual key
try:
...
def process_speech_buffer():
    ...
    try:
        ...
        # Text to speech - Play directly through speakers
        ...
        try:
            ...
            # Fallback: save file if direct playback fails
            try:
                for i, (_, _, audio) in enumerate(tts_pipeline(response, voice='af_heart')):
                    # Convert to numpy array if it's a tensor
                    if hasattr(audio, 'detach'):  # It's a PyTorch tensor
                    ...
                    output_file = f"response_fallback_{int(time.time())}_{i}.wav"
                    sf.write(output_file, audio_np, sample_rate)
                    print(f"💾 Fallback: Saved {output_file}")
            except Exception as fallback_error:
                print(f"Both playback and file saving failed: {fallback_error}")
